...
// will become a million times cleaner
exports.properties = {
  insertBefore (val) {
    console.log('is this ever used???')
...
exports.define = {
  ...
  keysInternal (keys, field, check) {
    if (!keys && keys !== false) {
      ...
      if (keys && field === '_keys') {
        cacheIndex(this, keys)
      }
    }
    return keys
  },
  clearKeys (target) {
    this._staticElements = void 0
...
function cacheIndex (target, keys) {
  // do this in sort (one loop)
  for (let i = 0, len = keys.length; i < len; i++) {
    target[keys[i]].index = i
  }
}
