...
exports.types = {
  group: {
    ...
    define: {
      ...
      storeState (val, state, type, stamp, subs, tree, id, pid, nojoin, cb) {
        ...
        if (!store.stamp) {
          ...
          vstamp.on(stamp, () => {
            store.stamp = null
            console.log('exec and REMOVE! stamp')
            cb(store)
            // this.render.state()
          })
        }
        console.log('yoooo! store it!', stamp, state.path(), this.path())
        // Q: why always the join? isnt that limiting?
        // A: it's a common factor between all current group properties
    ...
    // nothing basiclly...
    // subscriptionType: 'done' // remove this
  }
}
