    ...
    child: {
      ...
      render: {
        ...
        state (target, s, type, stamp, subs, tree, id, pid, order) {
          var w = subs
          var p
          const key = target._parent.key
          const store = target.getStore(tree, pid + key)
          while (w && !p) {
            let _ = w._
            if (_) {
              p = w._.t && _.t[pid] || _.s && _.s[pid]
              w = _.p
            }
          }
          p = p[key]
          ...
          p.render.state(p, s, type, stamp, subs, tree, id, pid, order, store)
        }
      }
    }
  }
