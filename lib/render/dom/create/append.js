...
function findNode (uid, tree, target, nextchild) {
  ...
  if (!target.$) {
    console.error('no target$ in ordering must be the .$ isStatic null case', target)
    return
  }

  if (/^\$switch/.test(target.$[len - 1])) {
    len = len - 1
  }
  ...
  if (nextchild._$length > 1) {
    ...
    let key = nextchild$[0]
    if (/^\$test/.test(key)) {
      console.log('  --- special test case $test in order -- fix this since this is def wrong!')
