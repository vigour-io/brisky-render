...
module.exports = function render (elem, state, callback) {
  ...
  if (state === void 0) {
    ...
    if (callback) {
      callback(state, 'new', state._lstamp, subs, tree, subs, tree)
      subscribe(state, subs, function (state, type, stamp, nsubs, ntree, sType) {
        renderMultiple(state, type, stamp, nsubs, ntree, sType)
        callback(state, type, stamp, nsubs, ntree, sType, subs, tree)
      }, tree)
    } else {
