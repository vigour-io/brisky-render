...
module.exports = function renderState (target, state, type, stamp, subs, tree, id, pid, order) {
  if (type === 'remove') {
  ...
  return target.render.state(target, state, type, stamp, subs, tree, id, pid, order)
}

function emitRemove (target, state, tree, id, stamp) {
  const removeEmitter = target._emitters.removeEmitter
  if (removeEmitter) {
