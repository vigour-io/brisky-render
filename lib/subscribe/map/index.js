...
exports.define = {
  $map (map, prev) {
    ...
    this.isStatic = null
    if (this.$) {
      if (this._emitters && this._emitters.fn && this._emitters.fn.keys() > 0) {
        if (!this._emitters.removeEmitter || this._emitters.fn.keys() > 1) {
          // make a hook for this.. exclude remove
          this.set({ storeState: true }, false, true)
        }
      }

      if (this.$[0] === '$root' && (!map || !map._ || !map._.p)) {
        this.$.shift()
      }
