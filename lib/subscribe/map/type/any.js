...
module.exports = function collection (target, map) {
  ...
  if ($.length !== 1) {
    ...
    val.$remove = true
    console.error(path, val, map)
    map = merge(path, val, map)
    console.error('result', path, val, map)
  } else {
  ...
  console.log('2 -xpok???', $, map)

  iterator(target, map)

  console.error('2 result', map)

  subscriber(map, target, 't')

  console.error('3 result', map)
